...
def compute_temporal_loss(state, current_epoch, test_loader_in, test_loader_cor, device, current_city):
    """
    ...
    current_id_ac = compute_average_confidence(net, test_loader_in, device=device)
    current_cor_ac = compute_average_confidence(net, test_loader_cor, device=device)

    # Check if we have data from the previous city
    if current_city > 0:
        # Get average values from the previous city
        prev_city_ac_avg = state['city_avg_confidence'][-1]
        prev_city_cor_ac_avg = state['city_avg_cor_confidence'][-1]

        print(f"Comparing against City {current_city-1} averages: ID AC={prev_city_ac_avg:.3f}")
        print(f"Comparing against City {current_city-1} averages: COR AC={prev_city_cor_ac_avg:.3f}")
        # Compute changes against previous city averages
        id_ac_change = abs(current_id_ac - prev_city_ac_avg)
        cor_ac_change = abs(current_cor_ac - prev_city_cor_ac_avg)

        temporal_delta_ac = 0.05
        
        # Apply temporal penalties
        id_ac_penalty = max(0.0, id_ac_change - temporal_delta_ac)
        cor_ac_penalty = max(0.0, cor_ac_change - temporal_delta_ac)

        temporal_lambda = 1
        temporal_loss = temporal_lambda * (id_ac_penalty + cor_ac_penalty)
        ...
        # Log metrics
        state['id_ac_changes'].append(id_ac_change)
        state['cor_ac_changes'].append(cor_ac_change)
        state['temporal_losses'].append(temporal_loss.item())
        
        wandb.log({
            'temporal_loss': temporal_loss.item(),
            'id_ac_change': id_ac_change,
            'cor_ac_change': cor_ac_change,
        ...
        print(f"  Current ID AC: {current_id_ac:.3f}, Prev City Avg ID AC: {prev_city_ac_avg:.3f}")
        print(f"  Current COR AC: {current_cor_ac:.3f}, Prev City Avg COR AC: {prev_city_cor_ac_avg:.3f}")
...
def train(epoch, train_loader_in, train_loader_aux_in, train_loader_aux_in_cor, train_loader_aux_out, t, test_loader_in, test_loader_cor=None):
        ...
        temporal_loss = compute_temporal_loss(state, epoch, test_loader_in, test_loader_cor, device, t)
        ...
        for in_set, aux_in_set, aux_in_cor_set, aux_out_set in loaders:    
            ...
            if args.score == 'scone':
                ...
                loss = 0.20*loss_ce + 0.20*args.out_constraint_weight*loss_energy_out + 0.20*in_loss + 0.40*temporal_loss
