...
parser.add_argument('--model', '-m', type=str, default='mlp', # default allconv
                    choices=['allconv', 'wrn', 'densenet', 'mlp', 'deit'], help='Choose architecture.')
...
parser.add_argument('--in_constraint_weight', type=float, default=4,
...
#Temporal loss
parser.add_argument('--temporal_base_weight', type=float, default=0.2)
parser.add_argument('--temporal_max_drift', type=float, default=0.2)
...
import timm  # Add to your imports

if args.model == 'wrn':
    net = WideResNet(args.layers, num_classes, args.widen_factor, dropRate=args.droprate)
elif args.model in ['vit', 'deit']:
    net = timm.create_model('deit_small_patch16_224', pretrained=True, drop_path_rate=0.1)
    net.head = nn.Linear(net.head.in_features, num_classes)
    net.set_grad_checkpointing(enable=True)
...
# Only try loading WRN checkpoint if model is WRN
if args.load_pretrained == 'snapshots/pretrained' and args.model == 'wrn':
...
if args.score in ['energy_vos', 'woods', 'scone']:
    ...
    lr=state['learning_rate'], weight_decay=state['decay'])
...
from load_any_dataset import load_cifar, load_Imagenette, load_cinic10, load_clear_timestep_transform
...
city_0_loaders = load_clear_timestep_transform(timestep=0, use_vit=(args.model in ['vit', 'deit']))
city_1_loaders = load_clear_timestep_transform(timestep=1, use_vit=(args.model in ['vit', 'deit']))
city_2_loaders = load_clear_timestep_transform(timestep=2, use_vit=(args.model in ['vit', 'deit']))
city_3_loaders = load_clear_timestep_transform(timestep=3, use_vit=(args.model in ['vit', 'deit']))
city_4_loaders = load_clear_timestep_transform(timestep=4, use_vit=(args.model in ['vit', 'deit']))
city_5_loaders = load_clear_timestep_transform(timestep=5, use_vit=(args.model in ['vit', 'deit']))
city_6_loaders = load_clear_timestep_transform(timestep=6, use_vit=(args.model in ['vit', 'deit']))
city_7_loaders = load_clear_timestep_transform(timestep=7, use_vit=(args.model in ['vit', 'deit']))
city_8_loaders = load_clear_timestep_transform(timestep=8, use_vit=(args.model in ['vit', 'deit']))
city_9_loaders = load_clear_timestep_transform(timestep=9, use_vit=(args.model in ['vit', 'deit']))
